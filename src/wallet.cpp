...
/**
 * Fees smaller than this (in udco) are considered zero fee (for transaction creation)
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 DCO.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-dco-address
                    CScript scriptChange;
